...
# Chart and Data Placeholders (Step 1)
st.subheader("Real-Time Bandwidth Allocation")
chart_placeholder = st.empty()

st.subheader("Live Port Status")
...
if st.button("Start Live Simulation"):
    ...
    # Initialize empty history for the chart (Step 2)
    history_df = pd.DataFrame(columns=["Total Demand (Mbps)", "Total Allocated (Mbps)"])
    ...
    for tick in range(15):
        ...
        # Update Chart (Step 3)
        total_demand = sum(c.current_traffic_mbps for c in my_router.clients)
        total_allocated = sum(c.allocated_limit_mbps for c in my_router.clients)
        
        new_data = pd.DataFrame({
            "Total Demand (Mbps)": [total_demand],
            "Total Allocated (Mbps)": [total_allocated]
        })
        history_df = pd.concat([history_df, new_data], ignore_index=True)
        
        chart_placeholder.line_chart(history_df, y=["Total Demand (Mbps)", "Total Allocated (Mbps)"])
